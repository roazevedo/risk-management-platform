...
import { Risk, Control, ControlStatus, HistoryEntry } from '@/types';
import { CONTROL_TYPES, CONTROL_NATURES, CONTROL_RELATIONS, CONTROL_NEW_MODIFIED } from '@/src/constants/constants';
...
export default function ControlForm({ risk, control, onSave, onCancel }: ControlFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (control?.id && !justification.trim()) {
    ...
    try {
      ...
      // Lógica do Histórico
      let updatedHistory = [...(control?.history || [])];
      ...
      if (control?.id) {
        const newHistoryEntry: HistoryEntry = {
          ...
          changes: "Atualização de dados do controle"
        };
      ...
      alert("Erro ao salvar o controle.");
    } finally {
  ...
  return (
    <div className="max-h-[70vh] overflow-y-auto bg-gray-50 dark:bg-gray-800">
      <form onSubmit={handleSubmit} className="space-y-4 p-4">
        ...
        {/* Campo de Justificativa - só aparece na edição */}
        {control?.id && (
          <div className="mt-6 bg-yellow-50 dark:bg-yellow-900/20 p-4 rounded border border-yellow-200 dark:border-yellow-700">
            <label htmlFor="c-justification" className="block text-sm font-bold mb-1 text-yellow-800 dark:text-yellow-200">
        ...
        <div className="flex justify-end gap-4 pt-4 mt-4 border-t border-gray-200 dark:border-gray-700">
          <button
            ...
            className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 dark:bg-gray-600 dark:text-gray-200 dark:hover:bg-gray-500 transition-colors disabled:opacity-50"
          >
            Cancelar
          </button>
          <button
