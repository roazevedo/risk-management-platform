...
import { Process, HistoryEntry } from '@/types'; // Importe HistoryEntry
...
export default function ProcessForm({ process, onSave, onCancel }: ProcessFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (process?.id) {
                ...
                // Adiciona no início da lista (mais recente primeiro) ou no final
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-4 max-h-[80vh] overflow-y-auto p-2 custom-scrollbar">
            ...
            {/* ... CAMPOS EXISTENTES (Nome, Setor, etc) ... */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    <label htmlFor="p-sector" className="block text-sm font-medium mb-1 dark:text-gray-200">Setor *</label>
                    <input id="p-sector" type="text" name="sector" value={formData.sector} onChange={handleChange} required className={formInputStyle} />
                </div>
                <div>
                    <label htmlFor="p-manager" className="block text-sm font-medium mb-1 dark:text-gray-200">Gestor *</label>
            ...
            <div className="space-y-4">
                <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-200">Servidores (separados por vírgula)</label>
                    <input type="text" value={formData.responsibleServers?.join(', ')} onChange={(e) => handleArrayChange(e, 'responsibleServers')} className={formInputStyle} />
                </div>
                <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-200">Sistemas (separados por vírgula)</label>
                    <input type="text" value={formData.systemsUsed?.join(', ')} onChange={(e) => handleArrayChange(e, 'systemsUsed')} className={formInputStyle} />
                </div>
                <div>
                    <label className="block text-sm font-medium mb-1 dark:text-gray-200">Stakeholders (separados por vírgula)</label>
                    <input type="text" value={formData.stakeholders?.join(', ')} onChange={(e) => handleArrayChange(e, 'stakeholders')} className={formInputStyle} />
                </div>
            </div>

            <div>
                <label htmlFor="p-legalBasis" className="block text-sm font-medium mb-1 dark:text-gray-200">Base Legal</label>
                <textarea id="p-legalBasis" name="legalBasis" value={formData.legalBasis} onChange={handleChange} rows={2} className={formInputStyle}></textarea>
            </div>

            {/* --- NOVO CAMPO: JUSTIFICATIVA (Aparece apenas na Edição) --- */}
            {process?.id && (
            ...
            <div className="flex justify-end gap-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                <button type="button" onClick={onCancel} disabled={isSubmitting} className="px-4 py-2 bg-gray-200 rounded-md hover:bg-gray-300 disabled:opacity-50 text-gray-800">Cancelar</button>
